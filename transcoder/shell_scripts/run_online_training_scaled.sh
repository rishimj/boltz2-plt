...
RUN_NAME="online_train_scaled_${RUN_TS}"
...
FASTA_PATH="/usr/scratch/rmanimaran8/boltz/examples/multi_protein.fasta"
...
echo "RUN_NAME=$RUN_NAME" | tee -a "$RUN_LOG"
echo "CHECKPOINT_DIR=$CHECKPOINT_DIR" | tee -a "$RUN_LOG"
echo "FASTA_PATH=$FASTA_PATH" | tee -a "$RUN_LOG"
echo "LAYERS=$LAYERS" | tee -a "$RUN_LOG"
echo "NUM_STEPS=$NUM_STEPS" | tee -a "$RUN_LOG"
...
action_success() {
  ...
  # Check that all layers trained (at least one checkpoint per layer)
  for layer in 0 8 16 24 32 40; do
...
while true; do
  ...
  if action_success; then
    ...
    echo "RUN_NAME=$RUN_NAME" >> "$STATUS_FILE"
